# Atreides → OpenCode Plugin: Master Migration Plan
...
### 11.2 Test Coverage Requirements (Match Atreides 466 Tests)

| Category | Test Count | Priority | Examples |
|----------|------------|----------|----------|
| **Security** | ~60 | Critical | |
| Path traversal protection | 6 | Critical | `../../../etc/passwd` blocked |
| Command injection | 15+ | Critical | Obfuscation detection |
| Prototype pollution | 8 | Critical | Deep merge safety |
| Template injection | 4 | High | No code execution |
| Log injection | 2 | High | Control char removal |
| DoS protection | 10+ | High | maxFiles, maxDepth limits |
| Input validation | 15+ | High | Length limits, type checks |
| **Core Functionality** | ~200 | High | |
| CLI commands | 50 | High | init, doctor, update |
| File operations | 120 | High | Read, write, permissions |
| Hook handling | 60 | High | Before/after, matchers |
| **Plugin Logic** | ~150 | Medium | |
| Template rendering | 81 | Medium | Agent files, skills |
| Settings merge | 45 | Medium | Deep merge, overrides |
| Workflow engine | ~25 | Medium | Phase transitions |
| **Integration** | ~56 | Medium | |
| E2E workflow | 35 | Medium | Full session tests |
| Security verification | 25 | High | Regression prevention |
...
*This document serves as the authoritative reference for the Atreides → OpenCode migration project. Updates should be tracked in the Document History section.*
